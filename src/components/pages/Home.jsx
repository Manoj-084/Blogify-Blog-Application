...
const Home = () => {
    ...
    try {
      ...
      if (success) {
        ...
        if (data.blogs) {
          ...
          setFeaturedBlogs(data?.blogs.slice(0));
        }
  ...
  const getLatestBlogs = async () => {
    try {
      const { data, message, success } = await axios.get("/blogs/get-blogs?limit=9")
      ...
      <section className="featured-blogs py-12 ">
        <div className="container mx-auto px-4 max-w-[1440px],">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {mainFeatured && (
              <article className="group cursor-pointer" key={`featured-${mainFeatured.id}`}>
